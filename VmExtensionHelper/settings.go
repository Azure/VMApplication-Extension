...
func unmarshalProtectedSettings(ctx log.Logger, configFolder string, hs handlerSettings) (map[string]interface{}, error) {
	...
	if hs.SettingsCertThumbprint == "" {
		ctx.Log("message", "parseHandlerSettingsFile failed", "error", ErrNoCertificateThumbprint)
		return nil, ErrNoCertificateThumbprint
	}
	...
	if err != nil {
		...
		return nil, ErrInvalidProtectedSettingsData
	}

	v, err := decryptProtectedSettings(configFolder, hs.SettingsCertThumbprint, decoded)
	return v, err
}
...
func parseHandlerSettingsFile(ctx log.Logger, path string) (h handlerSettings, _ error) {
	...
	if err != nil {
		...
		return h, ErrInvalidSettingsFile
	}
	...
	if len(f.RuntimeSettings) != 1 {
		...
		return h, ErrInvalidSettingsRuntimeSettingsCount
	}
