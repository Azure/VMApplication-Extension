...
func Test_setSequenceNumberInternalNoRegistryKey(t *testing.T) {
	...
	err := setSequenceNumberInternal(ext, 42)
	require.Error(t, err, errNotFound)
}
...
func Test_setSequenceNumberInternalValidReplace(t *testing.T) {
	...
	sn, err := getSequenceNumberInternal(ext.Name, ext.Version)
...
func Test_decryptSettingsCertNotFound(t *testing.T) {
	...
	require.Error(t, err, ErrCertWithThumbprintNotFound)
}
...
func Test_decryptSettingsMisencoded(t *testing.T) {
	...
	require.Error(t, err, ErrInvalidProtectedSettingsData)
}
