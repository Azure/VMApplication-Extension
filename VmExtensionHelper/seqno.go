...
type sequenceNumberRetriever interface {
	getSequenceNumber(name string, version string) (uint, error)
}

type prodSequenceNumberRetriever struct {
}

func (prodSequenceNumberRetriever) getSequenceNumber(name string, version string) (uint, error) {
...
func getCurrentSequenceNumber(ctx log.Logger, retriever sequenceNumberRetriever, name string, version string) (sn uint, _ error) {
...
func findSeqNum(ctx log.Logger, configFolder string) (uint, error) {
	...
	if len(names) == 0 {
		...
		if err != nil {
			...
			return 0, ErrInvalidSettingsFileName
		}
